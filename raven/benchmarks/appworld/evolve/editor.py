...
from raven.benchmarks.appworld.evolve.eval import Candidate
from raven.benchmarks.appworld.evolve.sandbox import Sandbox
...
raven/benchmarks/appworld/agent_cli.py (it builds the agent prompt APPWORLD_PROMPT and \
runs one AppWorld task through a minimal AgentLoop). You MAY edit or \
create files ONLY under: raven/benchmarks/appworld/ and raven/agent/ (prompts, \
tools, the agent loop, and hooks under raven/agent/hook/) — EXCEPT \
raven/benchmarks/appworld/evolve/ (the evolution machinery that scores you: any edit \
...
def bash_edit_candidate(
    ...
    *,
    ...
    history_text: str = "",
    guard_text: str = "",
    import_smoke_module: Optional[str] = "raven.benchmarks.appworld.agent_cli",
