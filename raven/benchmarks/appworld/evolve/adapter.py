...
line), not a comma list. Env-gated mechanisms default OFF so vanilla is
byte-identical; a candidate's activation is a comma-joined ``--env`` string.

Output layout (from the real ``batch.py`` / ``paired_verdict.py``): the out-dir
...
class AppWorldConfig:
    ...
    batch_module: str = "raven.benchmarks.appworld.batch"
...
def stability_from_out_dir(out_dir: str | Path) -> dict[str, TaskStability]:
    ...
    The orchestrator's cold start normally reads harbor trial-dirs via
    ...
    anchor. Buckets use the same ``_bucket_for`` as the harbor path. Reads the
